...
雖然損失函數與預訓練相同，但有幾個關鍵的實作細節與預訓練所使用的設定不同。許多實踐（例如決定使用哪種類型的平行處理來將模型切分到多個 GPU 上）與預訓練相同，只是使用的機器總數通常較少（這是基於下方列出的第一個技術變更）：

* 較小的批次大小：與預訓練相比，指令微調（以及其他後訓練技術如偏好微調）使用小得多的批次大小，以在較窄的資料分布上良好地最佳化，同時保留模型從預訓練獲得的泛化能力。例如，OLMo 2 的 7B 模型預訓練使用 1024 封裝列 (packed-rows) 的批次大小，13B 模型則使用 2048；這些模型的總上下文長度為 4096 個標記，且批次中的每一列都是填滿序列長度的文件組合。對於後訓練，這兩個模型僅使用 256 個提示的批次大小，而不需要填滿整個序列長度（因此每批次中非遮罩標記的數量要少得多）。較小的批次大小意味著這些訓練工作無法像預訓練那樣切分到那麼多設備上——在實務上，分散式訓練設定有每個設備的最小批次大小限制，因此如果你試圖為 SFT 保留較小的全局批次大小，你能使用的 GPU 總數會較少。實務上，批次大小迫使每個訓練工作分配較少同時運作的 GPU 並非限制因素，因為 SFT 的訓練標記數量遠小於預訓練，而且在後訓練中需要為多個種子 (seeds) 進行訓練，以獲得最佳的最終效能。

* 提示遮罩：在預訓練時，批次中的每個標記都以自迴歸方式進行預測，並將損失應用於它們。在指令微調中，提示標記會被遮罩，因此模型不會學習精確預測使用者查詢——而只會學習預測回應。這同樣適用於其他後訓練演算法。

* 多輪對話遮罩：對於多輪對話，常見的遮罩選擇有兩種。
	* (1) 僅最後一輪：損失僅包含最後一輪助理回覆的標記，而所有先前的上下文（包含先前的助理回覆）皆被遮罩。長對話仍可被「展開」成多個訓練樣本：對於 N 輪的對話，每個範例預測一個助理回應，同時遮罩所有先前的上下文並排除任何未來的對話輪次。
	* (2) 僅遮罩使用者輪次：所有使用者輪次皆被遮罩，但每個助理輪次皆包含在損失中。如果您希望獲得更多（較短）的訓練範例，您仍可在此設定中進行展開，但主要差異在於模型會直接針對中間的助理回覆進行訓練。

* 與預訓練相同的損失函數：指令微調使用與預訓練語言模型相同的自迴歸損失函數，但在資料處理、遮罩（僅針對完整序列進行訓練，而預訓練文件可拆分至不同批次）等方面有實質差異。

* 學習率：SFT 通常使用比預訓練小一到兩個數量級的學習率，以最佳地管理不同的最佳化動態（較小的資料集、較小的批次，以及強大的預訓練初始化，都傾向於較保守的更新）。例如，OLMo 2 預訓練的峰值學習率為 $3\times10^{-4}$，而 SFT 則為 $1\times10^{-5}$ 。OLMo 3 使用較高的 SFT 學習率 $5-8\times10^{-5}$ ，部分原因是其訓練基礎設施使用了序列封裝 (sequence packing) 技術，將多個範例裝入每個訓練序列中，從而增加了以有用標記衡量之有效批次大小。較大的批次會產生變異較低的梯度估計，進而支援較高的學習率而不會破壞訓練穩定性——這種關係被稱為線性縮放法則。通常會在訓練步驟的一小部分內對學習率進行預熱 (warmed up)，然後線性衰減。在實務上，團隊通常會掃描多個學習率，並在保留的評估套件上選擇最佳的檢查點。

# 5 獎勵模型 reward modeling

> The practice of reward modeling for RLHF is closely related to inverse reinforcement learning, where the problem is to approximate an agent's reward function given trajectories of behavior, and other areas of deep reinforcement learning

獎勵模型是現代 RLHF 方法的核心，因為它是學習複雜人類偏好的所在 它們使我們的模型能夠從難以具體指定的訊號中學習 它們將資料中複雜的特徵壓縮成可用於下游訓練的表示形式，這是一種魔法，再次展現了現代深度學習的複雜能力 這些模型作為核心最佳化的代理目標，如同後續章節所研究的內容 如圖 11 所示，獎勵模型扮演著類似標準強化學習環境的角色，為代理提供學習訊號，然而與固定的環境不同，我們得以從人類偏好中學習它

獎勵模型在歷史上已被廣泛應用於強化學習研究中，作為環境獎勵的代理 [54] 現代形式的獎勵模型被提出作為研究價值對齊問題的工具 [38] 這些模型傾向於接收某種輸入並輸出單一的純量獎勵值 這種獎勵可以有多種形式，在傳統的強化學習問題中，它試圖逼近該問題確切的環境獎勵，但在 RLHF 中我們將會看到，獎勵模型實際上輸出的是特定輸入屬於高品質的機率（即在成對偏好關係中被選中的答案） RLHF 中獎勵建模的實踐與逆向強化學習密切相關，在逆向強化學習中，問題在於給定行為軌跡來逼近代理的獎勵函數 [71]，且與深度強化學習的其他領域相關 高層次的問題陳述是相同的，不過實作與重點領域完全不同，因此它們通常被視為完全獨立的研究領域

最常見的獎勵模型通常被稱為 Bradley-Terry 獎勵模型，也是本章的主要重點，它預測一段文本接近訓練比較中偏好文本的機率 在本節稍後，我們也將這些模型與結果獎勵模型 (ORMs)、過程獎勵模型 (PRMs) 以及其他類型的獎勵模型進行比較
...
Bradley-Terry 模型假設每個項目具有潛在強度 $p_{i}>0$，且觀察到的偏好是這些潛在強度的雜訊反映。通常會使用無界分數對 Bradley-Terry 模型重新參數化，其中 $p_{i}=e^{r_{i}}$，這會得出以下形式：

$$P(i>j)=\frac{e^{r_{i}}}{e^{r_{i}}+e^{r_{j}}}=\sigma(r_{i}-r_{j}).$$

只有分數的差異才有意義：對所有 $T_{i}$ 加上相同的常數，會使 $P(i>j)$ 保持不變。這些形式缺乏自然法則的地位，是人類偏好的有用近似值，通常在 RLHF 中運作良好。

為了訓練獎勵模型，我們必須制定一個滿足上述關係的損失函數。在實務上，這透過將語言模型轉換為輸出純量分數的模型來完成，通常經由一個小型的線性頭，從模型的最終隱藏狀態產生單一獎勵值。給定一個提示 x 與兩個採樣的補全 $y_{1}$ 和 $y_{2}$，我們使用獎勵模型 $r_{\theta}$ 對兩者進行評分，並將條件分數寫為 $r_{\theta}(y_{i}|x)$。

獎勵模型分配給 $y_{1}$ 被偏好於 $y_{2}$ 的機率變成：

$$P(y_{1}>y_{2}|x)=\frac{exp(r_{\theta}(y_{1}|x))}{exp(r_{\theta}(y_{1}|x))+exp(r_{\theta}(y_{2}|x))}.$$

我們將偏好的補全表示為 $y_{c}$（selective），並將拒絕的補全表示為 $y_{r}$。產生的損失促使獎勵模型為人類偏好的補全分配高於被拒絕補全的分數，並使用 sigmoid 將分數差異轉換為機率。透過最大化上述函數的對數概似（或替代地最小化負對數概似），我們可以得出以下損失來訓練獎勵模型：
...
$$\mathcal{L}(\theta)=-log(\sigma(r_{\theta}(y_{c}|x)-r_{\theta}(y_{r}|x)))$$

第二種是使用 softplus 函數 $log(1+e^{x})$ 表達的數學等價形式，如其他研究所述：

$$\mathcal{L}(\theta)=log(1+e^{r_{\theta}(y_{r}|x)-r_{\theta}(y_{c}|x)})$$

這些形式是等價的，方法是令 $\Delta=r_{\theta}(y_{c}|x)-r_{\theta}(y_{r}|x)$ 並使用 $\sigma(\Delta)=\frac{1}{1+e^{-\Delta}}$，這意味著 $-log~\sigma(\Delta)=log(1+e^{-\Delta})=log(1+e^{r_{\theta}(y_{r}|x)-r_{\theta}(y_{c}|x)})$。它們兩者皆出現在 RLHF 文獻中。