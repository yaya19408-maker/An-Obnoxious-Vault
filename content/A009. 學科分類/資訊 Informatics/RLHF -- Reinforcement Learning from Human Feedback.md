...
獎勵模型在歷史上已被廣泛應用於強化學習研究中，作為環境獎勵的代理 [54] 現代形式的獎勵模型被提出作為研究價值對齊問題的工具 [38] 這些模型傾向於接收某種輸入並輸出單一的純量獎勵值這種獎勵可以有多種形式，在傳統的強化學習問題中，它試圖逼近該問題確切的環境獎勵，但在 RLHF 中我們將會看到，獎勵模型實際上輸出的是特定輸入屬於高品質的機率（即在成對偏好關係中被選中的答案） RLHF 中獎勵建模的實踐與逆向強化學習密切相關，在逆向強化學習中，問題在於給定行為軌跡來逼近代理的獎勵函數 [71]，且與深度強化學習的其他領域相關 高層次的問題陳述是相同的，不過實作與重點領域完全不同，因此它們通常被視為完全獨立的研究領域
